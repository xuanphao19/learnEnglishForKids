  ...
  var defaults = {
    ...
    methods = {
      play: function (element) {
        $tgt = element;
        currentTrack = _methods.getFileNameWithoutExtension(
          $tgt.attr("media-url")
        );
        if (lesson_child()) {
          currentTrack = _methods.getFileNameWithoutExtension(
            checkGOLDENKIDSaudioLink($tgt.attr("media-url"))
          );
        } else {
          if (defaults.isPaid == true) {
            currentTrack = _methods.getFileNameWithoutExtension(
              checkVIPaudioLink($tgt.attr("media-url"))
            );
          }
        }
        isPlaying = true;
    ...
    _methods = {
      ...
      updateTrackState: function (evt) {
        ...
        if (
          ...
          (audio &&
            currentTrack !==
              _methods.getFileNameWithoutExtension($tgt.attr("media-url")))
        ) {
          methods.play($tgt);
        } else if (!isPlaying) {
          methods.resume();
...
function PlayingAudioHide(element) {
  if (element.hasClass("uba_hideAudio")) {
    element
      ...
      .attr(
        "src",
        "/file/dungchung/audioPlayer/ubanplayer/img/speaker-playing.gif"
      );
...
function stopAudioHide(element) {
  if (element.hasClass("uba_hideAudio")) {
    element
      ...
      .attr(
        "src",
        "/file/dungchung/audioPlayer/ubanplayer/img/speaker-stop.gif"
      );
  }
}
...
function DetectChrome() {
  if (uagent.search(browserChrome) > -1) {
    return true;
  } else {
    return false;
  }
...
function DetectBlackBerryTouch() {
  if (
    DetectBlackBerry() &&
    (uagent.search(deviceBBStorm) > -1 || uagent.search(deviceBBTorch) > -1)
  )
    return true;
  else return false;
}
...
function DetectKindle() {
  if (uagent.search(deviceKindle) > -1) return true;
  else return false;
}
